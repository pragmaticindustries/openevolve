# EVOLVE-BLOCK-START
"""Ultra-fast, parallelized, hash-deduped, hybrid search for optimal topology constellation (faster, leaner version)."""
...
# Avoid global variable where possible, but keep for node id assignment
node_counter = 0
...
def apply_generic_optimizer(
    ...
    # Estimate max search depth based on device count
    ...
    # Fast filtering: device types with >3 ports are "distributors"
    ...
    # - beam_width and max_nodes_per_level are kept tight to minimize node explosion
    # - Only best 1 per local area is kept per expansion (aggressive pruning)
    # - Greedy fallback is early
    traveller = NodeEvaluator(
        ...
        top_candidates_per_area=1,
        greedy_depth=1,       # Switch to greedy after 1 expansion
        parallel=True,
        parallel_workers=8
    )
...
class Node:
    ...
    def uuid_dict(self):
        # Traverse up, record unique (local_area, device_type.uuid) for dedup hash
        devices = {}
        current = self
...
class NodeEvaluator:
    """Hybrid beam-greedy search with parallel expansion and aggressive pruning."""
    ...
    def beam_greedy_search(
        ...
        for d in range(max_depth):
            # Early switch to greedy after greedy_depth
    ...
    def greedy_expand(self, nodes: List[Node], remaining_depth: int, weights: Optional[List[float]]) -> Optional[Node]:
        # Greedy: follow best child at each step to terminal
        best_terminal = None
        best_cost = float("inf")
        for node in nodes:
            current = node
    ...
    def expand_node(self, node: Node, weights: Optional[List[float]] = None) -> List[Node]:
        ...
        for local_area in local_areas:
            ...
            def candidate_worker(dt) -> Optional[Node]:
                ...
                normalize_costs(child, weights)
                # Early pruning: only keep if connections increased
                if len(child.topology.connections) == len(node.topology.connections):
                    child.no_new_connections = True
                    return None
                return child

            # Parallelize candidate expansion if worthwhile
            ...
            # Only keep the best N candidates in this local area
            if candidate_nodes:
