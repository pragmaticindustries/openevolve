...
class NodeEvaluator:
    ...
    def traverse_breadth_first(
        ...
        """Breadth-first search with beam search and cost-based node expansion."""
        import heapq
        ...
        for d in range(max_depth):
            ...
            n_queue = len(queue)
            print(
                f"Expanding at depth {d + 1}/{max_depth} ({100.0 * d / max_depth:.1f}%), inspecting {n_queue} active edges"
            )

            # Early exit: If all nodes in queue are terminal, stop expanding
            if all(n.is_terminal() for n in queue):
                print("All nodes terminal at current depth, breaking early.")
                break

            # Expand only non-terminal nodes, prioritize those with lower cost
            # Sort queue by normalized_cost if available, else fallback to 0
            queue_sorted = sorted(
                queue, key=lambda n: getattr(n, "normalized_cost", 0)
            )
            # Optionally: Only expand top-k most promising nodes at this depth (beam search)
            expand_candidates = queue_sorted[:beam_width]

            for num, edge in enumerate(expand_candidates):
                if edge.is_terminal():
                    continue

                progress = 100 * num / n_queue if n_queue else 100
                if (
                    self.update_task_state
                    and (num == 0 or num == n_queue - 1 or num % max(1, n_queue // 20) == 0)
                ):
                    time_left = None
                    self.update_task_state(
                        {
                ...
                # Expand edge by 1 depth, collect its children
                self.traverse_depth_first(edge, 1)
                next_queue.extend(edge.children)
            print("")

            # Prune: keep only top 'beam_width' nodes based on cost
            if next_queue:
                normalize_costs(root, debug_weights)
                # Only consider not-terminal nodes for next beam (to allow exploration)
                candidates = [n for n in next_queue if not n.is_terminal()]
                if not candidates:
                    candidates = next_queue  # fallback
                # Use heapq.nsmallest for efficiency
                queue = heapq.nsmallest(beam_width, candidates, key=lambda node: node.normalized_cost)
            else:
                queue = []
    ...
    def traverse_depth_first(
        ...
        # Early duplicate detection: skip if already visited
        ...
        # Filter device types ONCE per call, not per device type
        device_types_per_area = {}
        for local_area in local_areas:
            ...
            area_types = [
                device_type
                for device_type in area_types
                if device_type.housing_material == parameters.distributor_housing_material
            ]
            device_types_per_area[local_area] = area_types

        # Heuristic: try fewer promising device types first (sort by #ports descending, as a simple proxy for coverage)
        # Further optimization: try only the most promising k device types per area (reduce branching factor)
        beam_k_per_area = 3  # Only expand top 3 device types per area

        for local_area, device_types in device_types_per_area.items():
            # Sort device types by number of ports descending, then by identifier to ensure deterministic order
            device_types_sorted = sorted(device_types, key=lambda dt: (-len(dt.ports), dt.identifier))
            # Only expand the most promising types for this area
            for dt in device_types_sorted[:beam_k_per_area]:
                ...
                # Check for duplicate in visited set
